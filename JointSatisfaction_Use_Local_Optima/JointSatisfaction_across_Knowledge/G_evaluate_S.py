...
# mp version
def func(N=None, K=None, agent_num=None, specialist_expertise=None, search_iteration=None, loop=None, return_dict=None, sema=None):
    np.random.seed(None)
...
if __name__ == '__main__':
    ...
    for specialist_expertise in specialist_expertise_list:
        ...
        with open("gs_joint_satisfaction_across_K_S_{0}".format(specialist_expertise), 'wb') as out_file:
            pickle.dump(joint_confusion_across_K, out_file)
    t1 = time.time()
    now = datetime.datetime.now()
    print(now.strftime("%Y-%m-%d %H:%M:%S"))
    print("Joint Satisfaction GS: ", time.strftime("%H:%M:%S", time.gmtime(t1-t0)))
