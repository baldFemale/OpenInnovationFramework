...
if __name__ == '__main__':
    ...
    for maturity_threshold in maturity_threshold_list:
        ...
        with open("gs_maturity_threshold_{0}_interval_{1}_pairwise_diversity_across_K_size_{2}".format(
                maturity_threshold, visibility_interval, agent_num), 'wb') as out_file:
    ...
    now = datetime.datetime.now()
    print(now.strftime("%Y-%m-%d %H:%M:%S"))
    print("GS Maturity-Based Visibility with Interval {0}: ".format(visibility_interval),
