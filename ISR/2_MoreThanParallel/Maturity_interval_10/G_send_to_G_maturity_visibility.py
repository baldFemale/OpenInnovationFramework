...
def func(N=None, K=None, agent_num=None, search_iteration=None, visibility_prob=None,
    ...
    - The sender crowd only searches and shares mature, visible full solutions.
    ...
    Visibility condition:
        ...
        when activated, disclose a sender's solution if:
            agent.visibility_status is True and agent.cog_fitness >= maturity_threshold.

    Interpretation:
        visibility_prob = baseline visibility intensity.
            In the maturity-threshold experiment, this is usually fixed at 1.0
            so that maturity_threshold is the focal variable.

        maturity_threshold = minimum cognitive fitness required for disclosure.
    ...
    for period in range(search_iteration):
        ...
        if (period + 1) % visibility_interval == 0:
            ...
            # The sender crowd discloses complete solution strings, conditional
            # on fixed visibility status and the maturity threshold.
            ...
            for agent in crowd_sender.agents:
                if agent.visibility_status and agent.cog_fitness >= maturity_threshold:
                    ...
                    crowd_sender.solution_pool.append([domains, solution])
...
if __name__ == '__main__':
    ...
    # Baseline visibility degree is fixed so that maturity threshold is the focal variable.
    # visibility_prob = 1.0 means all sender solutions are structurally visible,
    # subject to the maturity-threshold condition during each visibility period.
    visibility_prob = 1.0

    # Visibility frequency: mature sender solutions become visible every x periods.
    ...
    for maturity_threshold in maturity_threshold_list:
        ...
        for K in K_list:
            ...
            for loop in range(landscape_iteration):
                ...
                p = mp.Process(target=func, args=(N, K, agent_num, search_iteration, visibility_prob,
                                                  maturity_threshold, visibility_interval, loop,
                                                  return_dict, sema))
                jobs.append(p)
                p.start()
        ...
        with open("gg_maturity_threshold_{0}_interval_{1}_breakthrough_fitness_across_K_size_{2}".format(
                maturity_threshold, visibility_interval, agent_num), 'wb') as out_file:
            pickle.dump(breakthrough_fitness_across_K, out_file)

        with open("gg_maturity_threshold_{0}_interval_{1}_breakthrough_rank_across_K_size_{2}".format(
                maturity_threshold, visibility_interval, agent_num), 'wb') as out_file:
            pickle.dump(breakthrough_rank_across_K, out_file)

        with open("gg_maturity_threshold_{0}_interval_{1}_diversity_across_K_size_{2}".format(
                maturity_threshold, visibility_interval, agent_num), 'wb') as out_file:
            pickle.dump(diversity_across_K, out_file)

        with open("gg_maturity_threshold_{0}_interval_{1}_pairwise_diversity_across_K_size_{2}".format(
                maturity_threshold, visibility_interval, agent_num), 'wb') as out_file:
    ...
    now = datetime.datetime.now()
    print(now.strftime("%Y-%m-%d %H:%M:%S"))
    print("GG Maturity-Based Visibility with Interval {0}: ".format(visibility_interval),
