...
def func(N=None, K=None, agent_num=None, search_iteration=None, uniform_sharing_prob=None,
    ...
    for period in range(search_iteration):
        ...
        # but only subjectively mature G solutions are disclosed to S.
...
if __name__ == '__main__':
    ...
    for maturity_threshold in maturity_threshold_list:
        ...
        with open("gs_maturity_threshold_{0}_breakthrough_fitness_across_K_size_{1}".format(
                maturity_threshold, agent_num), 'wb') as out_file:
            pickle.dump(breakthrough_fitness_across_K, out_file)

        with open("gs_maturity_threshold_{0}_breakthrough_rank_across_K_size_{1}".format(
                maturity_threshold, agent_num), 'wb') as out_file:
            pickle.dump(breakthrough_rank_across_K, out_file)

        with open("gs_maturity_threshold_{0}_diversity_across_K_size_{1}".format(
                maturity_threshold, agent_num), 'wb') as out_file:
    ...
    now = datetime.datetime.now()
    print(now.strftime("%Y-%m-%d %H:%M:%S"))
    print("GS Maturity-Based Visibility: ", time.strftime("%H:%M:%S", time.gmtime(t1 - t0)))
