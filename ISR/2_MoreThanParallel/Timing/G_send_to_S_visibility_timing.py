...
# @Author   : Junyi
# @FileName: G_send_to_S_visibility_timing.py
# @Software : PyCharm
...
if __name__ == '__main__':
    ...
    for visibility_start in visibility_start_list:
        ...
        with open("gs_visibility_start_{0}_interval_{1}_breakthrough_fitness_across_K_size_{2}".format(
                visibility_start, visibility_interval, agent_num), 'wb') as out_file:
            pickle.dump(breakthrough_fitness_across_K, out_file)

        with open("gs_visibility_start_{0}_interval_{1}_breakthrough_rank_across_K_size_{2}".format(
                visibility_start, visibility_interval, agent_num), 'wb') as out_file:
            pickle.dump(breakthrough_rank_across_K, out_file)

        with open("gs_visibility_start_{0}_interval_{1}_diversity_across_K_size_{2}".format(
                visibility_start, visibility_interval, agent_num), 'wb') as out_file:
            pickle.dump(diversity_across_K, out_file)

        with open("gs_visibility_start_{0}_interval_{1}_pairwise_diversity_across_K_size_{2}".format(
                visibility_start, visibility_interval, agent_num), 'wb') as out_file:
    ...
    now = datetime.datetime.now()
    print(now.strftime("%Y-%m-%d %H:%M:%S"))
    print("GS Visibility Timing with Interval {0}: ".format(visibility_interval),
