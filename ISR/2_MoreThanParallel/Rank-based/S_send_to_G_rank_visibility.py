...
if __name__ == '__main__':
    ...
    for fitness_threshold in fitness_threshold_list:
        ...
        with open("sg_rank_based_fitness_threshold_{0}_breakthrough_fitness_across_K_size_{1}".format(
                fitness_threshold, agent_num), 'wb') as out_file:
            pickle.dump(breakthrough_fitness_across_K, out_file)

        with open("sg_rank_based_fitness_threshold_{0}_breakthrough_rank_across_K_size_{1}".format(
                fitness_threshold, agent_num), 'wb') as out_file:
            pickle.dump(breakthrough_rank_across_K, out_file)

        with open("sg_rank_based_fitness_threshold_{0}_diversity_across_K_size_{1}".format(
                fitness_threshold, agent_num), 'wb') as out_file:
    ...
    now = datetime.datetime.now()
    print(now.strftime("%Y-%m-%d %H:%M:%S"))
    print("SG Rank-Based Visibility: ", time.strftime("%H:%M:%S", time.gmtime(t1 - t0)))
