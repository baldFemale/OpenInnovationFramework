...
def func(N=None, K=None, agent_num=None, search_iteration=None, visibility_prob=None,
    ...
    # Sender crowd: Generalists who only search and make solutions visible
...
if __name__ == '__main__':
    ...
    for visibility_prob in visibility_prob_list:
        ...
        with open("gs_visibility_prob_{0}_interval_{1}_pairwise_diversity_across_K_size_{2}".format(
                visibility_prob, visibility_interval, agent_num), 'wb') as out_file:
            pickle.dump(pairwise_diversity_across_K, out_file)

        with open("gs_visibility_prob_{0}_interval_{1}_adopted_solution_fitness_across_K_size_{2}".format(
                visibility_prob, visibility_interval, agent_num), 'wb') as out_file:
            pickle.dump(adopted_solution_fitness_across_K, out_file)

        with open("gs_visibility_prob_{0}_interval_{1}_adopted_solution_fitness_across_time_across_K_size_{2}".format(
                visibility_prob, visibility_interval, agent_num), 'wb') as out_file:
