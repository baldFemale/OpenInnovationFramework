...
"""
Independent-search baseline for S crowds.
...
def func(N=None, K=None, agent_num=None, search_iteration=None,
    ...
    - One S crowd is created on the landscape.
...
if __name__ == '__main__':
    ...
    agent_num_list = np.arange(500, 1001, step=100, dtype=int).tolist()
    ...
    for agent_num in agent_num_list:
        ...
        with open("s_independent_breakthrough_fitness_across_K_size_{0}".format(agent_num), 'wb') as out_file:
            pickle.dump(breakthrough_fitness_across_K, out_file)

        with open("s_independent_breakthrough_rank_across_K_size_{0}".format(agent_num), 'wb') as out_file:
            pickle.dump(breakthrough_rank_across_K, out_file)

        with open("s_independent_diversity_across_K_size_{0}".format(agent_num), 'wb') as out_file:
            pickle.dump(diversity_across_K, out_file)

        with open("s_independent_pairwise_diversity_across_K_size_{0}".format(agent_num), 'wb') as out_file:
            pickle.dump(pairwise_diversity_across_K, out_file)

    t1 = time.time()
    now = datetime.datetime.now()
    print(now.strftime("%Y-%m-%d %H:%M:%S"))
    print("S Independent Search: ", time.strftime("%H:%M:%S", time.gmtime(t1 - t0)))
