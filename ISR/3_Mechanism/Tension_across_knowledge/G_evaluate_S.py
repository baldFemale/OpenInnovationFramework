    ...
    for specialist_expertise in specialist_expertise_list:
        ...
        for K in K_list:
            ...
            for loop in range(landscape_iteration):
                sema.acquire()
                p = mp.Process(target=func, args=(N, K, specialist_expertise, agent_num, search_iteration, loop, return_dict, sema))
                jobs.append(p)
                p.start()
            for proc in jobs:
                proc.join()
        ...
        with open("gs_joint_confirmation_across_knowledge_K", 'wb') as out_file:
            pickle.dump(joint_confirmation_across_knowledge, out_file)
        with open("gs_mutual_deviation_across_knowledge_K", 'wb') as out_file:
            pickle.dump(mutual_deviation_across_knowledge, out_file)
    t1 = time.time()
    now = datetime.datetime.now()
    print(now.strftime("%Y-%m-%d %H:%M:%S"))
    print("Interpretive Tension GS: ", time.strftime("%H:%M:%S", time.gmtime(t1-t0)))
