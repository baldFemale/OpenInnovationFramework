...
if __name__ == '__main__':
    ...
    with open("sg_joint_confirmation.pkl", 'wb') as out_file:
        pickle.dump(joint_confirmation_across_K, out_file)
    with open("sg_mutual_deviation.pkl", 'wb') as out_file:
        pickle.dump(mutual_deviation_across_K, out_file)
    t1 = time.time()
    now = datetime.datetime.now()
    print(now.strftime("%Y-%m-%d %H:%M:%S"))
    print("Interpretation Dynamics of SG: ", time.strftime("%H:%M:%S", time.gmtime(t1-t0)))
