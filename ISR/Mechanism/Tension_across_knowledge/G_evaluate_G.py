...
if __name__ == '__main__':
    ...
    for generalist_expertise in generalist_expertise_list:
        ...
        with open("gg_joint_confirmation_across_knowledge_K", 'wb') as out_file:
            pickle.dump(joint_confirmation_across_knowledge, out_file)
        with open("gg_mutual_deviation_across_knowledge_K", 'wb') as out_file:
            pickle.dump(mutual_deviation_across_knowledge, out_file)
    t1 = time.time()
    now = datetime.datetime.now()
    print(now.strftime("%Y-%m-%d %H:%M:%S"))
    print("Interpretive Tension GG: ", time.strftime("%H:%M:%S", time.gmtime(t1-t0)))
