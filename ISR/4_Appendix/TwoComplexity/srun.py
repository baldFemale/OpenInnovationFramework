...
def func(N=None, K=None, state_num=None, expertise_amount=None, agent_num=None, alpha=None,
    ...
    This keeps the original K x alpha experiment design in srun.py:
    ...
    try:
        ...
        crowd = Crowd(N=N, agent_num=agent_num, landscape=landscape, state_num=state_num,
                      generalist_expertise=0, specialist_expertise=expertise_amount, label="S")
...
if __name__ == '__main__':
    ...
    for alpha in alpha_list:
        ...
        with open("s_breakthrough_fitness_across_K_alpha_{0}".format(alpha), 'wb') as out_file:
            pickle.dump(breakthrough_fitness_across_K, out_file)

        with open("s_breakthrough_rank_across_K_alpha_{0}".format(alpha), 'wb') as out_file:
            pickle.dump(breakthrough_rank_across_K, out_file)

        with open("s_diversity_across_K_alpha_{0}".format(alpha), 'wb') as out_file:
            pickle.dump(diversity_across_K, out_file)

        with open("s_pairwise_diversity_across_K_alpha_{0}".format(alpha), 'wb') as out_file:
            pickle.dump(pairwise_diversity_across_K, out_file)

        with open("s_average_fitness_across_K_alpha_{0}".format(alpha), 'wb') as out_file:
