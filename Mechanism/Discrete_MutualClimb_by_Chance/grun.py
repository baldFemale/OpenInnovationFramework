...
def func(N=None, K=None, state_num=None, expertise_amount=None, agent_num=None,
    ...
    for _ in range(agent_num):
        ...
        for each_neighbor in neighbor_states:
            climbed_list = generalist.suggest_better_state_from_expertise(state=each_neighbor)
...
if __name__ == '__main__':
    ...
    with open("g_mutual_climb_across_K", 'wb') as out_file:
        pickle.dump(mutual_climb_across_K, out_file)
    t1 = time.time()
    print("G Climb: ", time.strftime("%H:%M:%S", time.gmtime(t1-t0)))
