...
def func(N=None, K=None, state_num=None, generalist_expertise=None, specialist_expertise=None, agent_num=None,
    ...
    for _ in range(agent_num):
        ...
        for _ in range(search_iteration):
            generalist.feedback_search(roll_back_ratio=0, roll_forward_ratio=roll_back)
...
if __name__ == '__main__':
    ...
    for roll_back in roll_back_list:
        ...
        with open("gg_performance_across_K_beta_{0}".format(roll_back), 'wb') as out_file:
            pickle.dump(performance_across_K, out_file)
        with open("gg_variance_across_K_beta_{0}".format(roll_back), 'wb') as out_file:
            pickle.dump(variance_across_K, out_file)
        with open("gg_first_quantile_across_K_beta_{0}".format(roll_back), 'wb') as out_file:
            pickle.dump(first_quantile_across_K, out_file)
        with open("gg_last_quantile_across_K_beta_{0}".format(roll_back), 'wb') as out_file:
            pickle.dump(lats_quantile_across_K, out_file)
        # retain time dimension
        with open("gg_performance_across_K_time_beta_{0}".format(roll_back), 'wb') as out_file:
            pickle.dump(performance_across_K_time, out_file)
        with open("gg_cog_performance_across_K_time_beta_{0}".format(roll_back), 'wb') as out_file:
            pickle.dump(cog_performance_across_K_time, out_file)
        with open("gg_variance_across_K_time_beta_{0}".format(roll_back), 'wb') as out_file:
            pickle.dump(variance_across_K_time, out_file)
        with open("gg_first_quantile_across_K_time_beta_{0}".format(roll_back), 'wb') as out_file:
            pickle.dump(first_quantile_across_K_time, out_file)
        with open("gg_last_quantile_across_K_time_beta_{0}".format(roll_back), 'wb') as out_file:
