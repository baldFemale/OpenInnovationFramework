...
if __name__ == '__main__':
    ...
    for roll_forward in roll_forward_list:
        for roll_back in roll_back_list:
            ...
            with open("gg_performance_across_K_forward_{0}_backward_{1}".format(roll_forward, roll_back), 'wb') as out_file:
                pickle.dump(performance_across_K, out_file)
            with open("gg_best_performance_across_K_forward_{0}_backward_{1}".format(roll_forward, roll_back), 'wb') as out_file:
                pickle.dump(best_performance_across_K, out_file)
            with open("gg_variance_across_K_forward_{0}_backward_{1}".format(roll_forward, roll_back), 'wb') as out_file:
                pickle.dump(variance_across_K, out_file)
            with open("gg_unique_diversity_across_K_forward_{0}_backward_{1}".format(roll_forward, roll_back), 'wb') as out_file:
                pickle.dump(unique_diversity_across_K, out_file)
            with open("gg_pair_wise_diversity_across_K_forward_{0}_backward_{1}".format(roll_forward, roll_back), 'wb') as out_file:
                pickle.dump(pair_wise_diversity_across_K, out_file)

    t1 = time.time()
    print("GG_2: ", time.strftime("%H:%M:%S", time.gmtime(t1 - t0)))
