...
def func(N=None, K=None, state_num=None, generalist_expertise=None, agent_num=None,
    ...
    for _ in range(agent_num):
        ...
        for _ in range(search_iteration):
            generalist.feedback_search(roll_back_ratio=roll_back, roll_forward_ratio=roll_forward)
...
if __name__ == '__main__':
    ...
    for roll_forward in roll_forward_list:
        for roll_back in roll_back_list:
            ...
            with open("sg_performance_across_K_forward_{0}_backward_{1}".format(roll_forward, roll_back), 'wb') as out_file:
                pickle.dump(performance_across_K, out_file)
            with open("sg_variance_across_K_forward_{0}_backward_{1}".format(roll_forward, roll_back), 'wb') as out_file:
                pickle.dump(variance_across_K, out_file)
            # retain time dimension
            with open("sg_performance_across_K_time_forward_{0}_backward_{1}".format(roll_forward, roll_back), 'wb') as out_file:
                pickle.dump(performance_across_K_time, out_file)
            with open("sg_cog_performance_across_K_time_forward_{0}_backward_{1}".format(roll_forward, roll_back), 'wb') as out_file:
                pickle.dump(cog_performance_across_K_time, out_file)
            with open("sg_variance_across_K_time_forward_{0}_backward_{1}".format(roll_forward, roll_back), 'wb') as out_file:
                pickle.dump(variance_across_K_time, out_file)

    t1 = time.time()
    print("Evaluating SG_5: ", time.strftime("%H:%M:%S", time.gmtime(t1-t0)))
