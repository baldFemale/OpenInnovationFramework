...
if __name__ == '__main__':
    ...
    for roll_forward in roll_forward_list:
        for roll_back in roll_back_list:
            ...
            with open("gs_performance_across_K_time_forward_{0}_backward_{1}".format(roll_forward, roll_back), 'wb') as out_file:
                pickle.dump(performance_across_K_time, out_file)
            with open("gs_cog_performance_across_K_time_forward_{0}_backward_{1}".format(roll_forward, roll_back), 'wb') as out_file:
                pickle.dump(cog_performance_across_K_time, out_file)
            with open("gs_variance_across_K_time_forward_{0}_backward_{1}".format(roll_forward, roll_back), 'wb') as out_file:
                pickle.dump(variance_across_K_time, out_file)

    t1 = time.time()
    print("Evaluating GS_3: ", time.strftime("%H:%M:%S", time.gmtime(t1-t0)))
