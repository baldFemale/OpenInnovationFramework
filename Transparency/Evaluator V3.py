...
class Evaluator:
    ...
    def generate_one_dimension_figure(self, dimension=None, title=None,y_label=None,show_variance=False, percentage=None, weighted_coverage=False):
        if title:
            self.title = title
        ...
        if ("Coverage" in y_label) and (not weighted_coverage):
            if not percentage:
        ...
        elif ("Coverage" in y_label) and weighted_coverage:
            fitness_weight = []
            dimension_files_list_2 = [[]] * len(file_name_reference)
            for each_file in selected_files_list:
                if "1Average" in each_file:
                    for index, each_reference in enumerate(file_name_reference):
                        if dimension == "Frequency":
                            if "_F" + str(each_reference) + '_' in each_file:
                                temp_2 = dimension_files_list_2[index].copy()
                                temp_2.append(each_file)
                                dimension_files_list[index] = temp_2
                        elif dimension == "Quality":
                            if "_Q" + str(each_reference) + '_' in each_file:
                                temp_2 = dimension_files_list_2[index].copy()
                                temp_2.append(each_file)
            ...
            for each_curve_files in dimension_files_list:
                print(each_curve_files)
                data_curve = self.load_data_from_folders(folder_list=each_curve_files)
                fitness_weight.append(data_curve)
            fitness_weight = np.array(fitness_weight, dtype=object)
                ...
                for k in range(all_curves_data.shape[1]):
                    for f in range(all_curves_data.shape[2]):
                        for l in range(all_curves_data.shape[3]):
                            ...
                            coverage_cache = []
                            for a in range(all_curves_data.shape[4]):
                                if all_curves_data[d][k][f][l][a] <= percentage * (self.state_num ** self.N) / 100:  # the rank
                                    if all_curves_data[d][k][f][l][a] not in coverage_cache:
                                        temp_coverage.append(fitness_weight[d][k][f][l][a])
                                        coverage_cache.append(all_curves_data[d][k][f][l][a])
                            # all_curves_data[d][k][f][l] = None
                            all_curves_data[d][k][f][l] = sum(temp_coverage)  # agent number
                            # print(temp_coverage)
