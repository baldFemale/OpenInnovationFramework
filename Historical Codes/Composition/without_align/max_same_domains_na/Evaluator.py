...
g_cog_performance_file = data_folder + r"\g_original_cog_performance_data_across_K"
...
g_cog_performance_2 = []
for each in g_cog_performance:
    g_cog_performance_2.append(sum(each) / len(each))
...
plt.plot(x, g_cog_performance_2, "k-", label="G_cog")
plt.xlabel('K', fontweight='bold', fontsize=10)
plt.ylabel('Cognition', fontweight='bold', fontsize=10)
plt.xticks(x)
plt.legend()
