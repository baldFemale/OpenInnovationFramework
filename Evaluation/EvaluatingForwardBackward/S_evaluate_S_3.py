...
if __name__ == '__main__':
    ...
    for roll_forward in roll_forward_list:
        for roll_back in roll_back_list:
            ...
            with open("ss_best_performance_across_K_forward_{0}_backward_{1}".format(roll_forward, roll_back), 'wb') as out_file:
            ...
            with open("ss_unique_diversity_across_K_forward_{0}_backward_{1}".format(roll_forward, roll_back), 'wb') as out_file:
                pickle.dump(unique_diversity_across_K, out_file)
            with open("ss_pair_wise_diversity_across_K_forward_{0}_backward_{1}".format(roll_forward, roll_back), 'wb') as out_file:
                pickle.dump(pair_wise_diversity_across_K, out_file)

    t1 = time.time()
    print("SS_3: ", time.strftime("%H:%M:%S", time.gmtime(t1 - t0)))
